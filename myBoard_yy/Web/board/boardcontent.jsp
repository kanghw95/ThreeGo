...
<script>
	...
	function updateContent(){
		...
		if($("#bpwd").val()==pass){
			var frm = document.getElementById("frm");
			frm.action="./board/boardupdate.jsp";
			frm.method="post";
			frm.submit();
		}else {
	...
	function deleteContent(){
		var pass = prompt("삭제 하고 싶으시면 패스워드를 입력하세요.");
		if($("#bpwd").val()==pass){
			var frm = document.getElementById("frm");
			frm.action="<%=request.getContextPath()%>/boarddelete.do";
			frm.method = "post";
			frm.submit();
		}else {
			alert("비밀번호가 일치하지 않습니다.");
...
<body>
	...
	<form id="frm">
		...
		<input type="hidden" id="bpwd" value="${boardcontent.bpwd }" >
		<input type="hidden" name="bwriter" value="${boardcontent.bwriter }" >
		<input type="hidden" name="bsubject" value="${boardcontent.bsubject }" >
		<table border="1">
			<tr>
				<td>작성자</td>
			...
			<tr>
				<td>파일선택</td>
				<td>
				<a href="<%=request.getContextPath() %>/files/${boardcontent.bfilepath }"
					download="${boardcontent.bfilepath }">${boardcontent.bfilepath }</a></td>
			</tr>
			<tr>
				<td colspan="2">
